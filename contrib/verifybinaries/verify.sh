...
###   pivotalcoincore.org and pivotalcoin.org and compares them.
...
WORKINGDIR="/tmp/pivotalcoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://pivotalcoincore.org"
HOST2="https://pivotalcoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="pivotalcoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'pivotalcoin-',
...
if [ $? -ne 0 ]; then
   echo "pivotalcoin.org failed to provide signature file, but pivotalcoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "pivotalcoin.org and pivotalcoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Pivotalcoin Core binary release signing key installed?"
   fi
