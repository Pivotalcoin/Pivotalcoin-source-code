This directory contains the PivotalcoinQT graphical user interface (GUI). It uses the cross-platform framework [Qt](https://www1.qt.io/developers/).
...
### pivotalcoingui.(h/cpp)

Represents the main window of the Pivotalcoin UI.
...
ClientModel is used by the main application `pivotalcoingui` and several models like `peertablemodel`.
...
Used to process BIP21 and BIP70 (see https://github.com/pivotalcoin/pivotalcoin/pull/11622) payment URI / requests. Also handles URI based application switching (e.g. when following a pivotalcoin:... link from a browser).
...
* UI elements like PivotalcoinAmountField, which inherit from QWidget.
* `pivotalcoinstrings.cpp`: automatically generated
* `pivotalcoinunits.(h/cpp)`: PVC / mPVC / etc handling
* `callback.h`
...
* don't change `local/pivotalcoin_en.ts`; this happens [automatically](/doc/translation_process.md#writing-code-with-translations)
...
4. Enter "pivotalcoin-qt" as project name, enter src/qt as location
5. Leave the file selection as it is
6. Confirm the "summary page"
7. In the "Projects" tab select "Manage Kits..."
8. Select the default "Desktop" kit and select "Clang (x86 64bit in /usr/bin)" as compiler
9. Select LLDB as debugger (you might need to set the path to your installation)
10. Start debugging with Qt Creator (you might need to the executable to "pivotalcoin-qt" under "Run", which is where you can also add command line arguments)
