 ...
 * between pivotalcoind, and pivotalcoin-qt and non-server tools.
*/
...
void Shutdown()
{
    ...
    RenameThread("pivotalcoin-shutoff");
...
#ifndef WIN32
    gArgs.AddArg("-pid=<file>", strprintf("Specify pid file. Relative paths will be prefixed by a net-specific datadir location. (default: %s)", PIVOTALCOIN_PID_FILENAME), false, OptionsCategory::OPTIONS);
#else
...
std::string LicenseInfo()
{
    ...
    const std::string URL_WEBSITE = "<https://pivotalcoin.org>";
...
static void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("pivotalcoin-loadblk");
...
/** Sanity checks
 *  Ensure that Pivotalcoin is running in a usable environment with all
...
{
    // Make sure only a single Pivotalcoin process is using the data directory.
...
bool AppInitMain()
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", PIVOTALCOIN_CONF_FILENAME)).string());
    LogPrintf("Using at most %i automatic connections (%i file descriptors available)\n", nMaxConnections, nFD);

    // Warn about relative -datadir path.
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if pivotalcoin is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if pivotalcoin is started while in a temporary directory.\n",
