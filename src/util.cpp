...
const char * const PIVOTALCOIN_CONF_FILENAME = "pivotalcoin.conf";
const char * const PIVOTALCOIN_PID_FILENAME = "pivotalcoind.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "pivotalcoind -foo=bar
...
#else
    const char* pszModule = "pivotalcoin";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Pivotalcoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Pivotalcoin
    // Mac: ~/Library/Application Support/Pivotalcoin
    // Unix: ~/.pivotalcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Pivotalcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Pivotalcoin";
#else
    // Unix
    return pathRet / ".pivotalcoin";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", PIVOTALCOIN_CONF_FILENAME);
...
fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", PIVOTALCOIN_PID_FILENAME)));
}
